...
3. Ensure the `<project> (main)` root session exists or route to its creation before placing initiation work beneath it.
4. Load project context when working inside or around an existing project folder.
5. For new projects, create the project directory and initialize the minimum structure using the approved stack and repository conventions.
6. For existing projects, verify the current repository structure, setup files, and documented operating guidance are usable.
7. Identify the minimum foundation needed before feature implementation can start.
8. Execute setup directly where safe; load exploration or architecture skills when the foundation depends on understanding an existing codebase or technical boundary.
9. Verify that the core project structure is in place and usable.
10. Report readiness and identify the first appropriate feature-delivery step.
...
If this is a new project repository and the setup workflow provides setup scripts, use them when they match the approved repository shape:
...
- Do not build product features during setup unless the setup step itself requires a tiny bootstrap artifact.
- Do not treat missing requirements as a setup problem.
- Keep the setup scope to foundation, readiness, and delivery structure.
