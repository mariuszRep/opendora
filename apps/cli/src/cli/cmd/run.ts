...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@projectflows/sdk/v2"
...
import { GlobTool, GrepTool, ListTool, ReadTool, EditTool, WriteTool } from "@projectflows/tools/filesystem"
import { WebFetchTool, CodeSearchTool, WebSearchTool } from "@projectflows/tools/browse-and-web"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.PROJECTFLOWS_AUTO_SHARE && !args.share) return
