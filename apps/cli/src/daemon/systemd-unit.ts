...
import type { ServiceRenderArgs } from "./service-types"
...
export function buildSystemdUnit({
  ...
  workingDirectory,
  environment,
}: ServiceRenderArgs): string {
  const execStart = programArguments.map(systemdEscapeArg).join(" ")
  const descriptionValue = description?.trim() || "OpenDora Server"
...
export function parseSystemdEnvAssignment(raw: string): { key: string; value: string } | null {
  ...
  return {
    key: unquoted.slice(0, eq).trim(),
    value: unquoted.slice(eq + 1).trim(),
  }
}
