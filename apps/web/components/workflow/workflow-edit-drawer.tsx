...
import type {
  ...
  JsonSchemaType,
  NodeModel,
} from "@/components/react-flow/unified-node"
...
const BUILTIN_SCHEMAS: Record<string, BuiltinSchema> = {
  ...
  decide: {
    ...
    properties: {
      prompt: {
        type: "string",
        description: "Decision prompt. Use $ctx.key for context. Agent must reply with one branch name.",
      },
    ...
    >
      {values.map((v, i) => (
        ...
        >
          {v}
          <X
            className="size-2.5 cursor-pointer opacity-60 hover:opacity-100"
            onClick={(e) => {
              e.stopPropagation()
...
export function WorkflowEditDrawer({
  ...
  const availableWorkflows = currentWorkflowId ? allWorkflows.filter((w) => w.id !== currentWorkflowId) : allWorkflows
  const agentList = useAgentList()

  // Tracked separately so useWorkflowParams always has a stable hook call
  const runWorkflowId =
    editingNodeData?.nodeType === NodeTypeId.RunWorkflow
  ...
  const handleSave = () => {
    if (editType === "node" && editingNodeData) {
      onSave({
        ...
        outputSchema:
          editingNodeData._schemaProps !== undefined
      ...
      case "node": {
        ...
        if (activeTab === "general") {
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  {(selectedSchema?.description ??
                    BUILTIN_SCHEMAS[editingNodeData.node.action_id ?? ""]?.description) && (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="What are these parameters for?"
                    rows={2}
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="What decision is this node making?"
                    rows={2}
                  />
                </div>
                <div className="space-y-2">
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="setworkdir-description"
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="Why is the working directory being changed here?"
                    rows={2}
                  />
                </div>
              </div>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="configsession-description"
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="Why is the session being reconfigured here?"
                    rows={2}
                  />
                </div>
              </div>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="foreach-description"
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="What is this loop doing?"
                    rows={2}
                  />
                </div>
                {onOpenForEachCanvas && (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="What values does this node define?"
                    rows={2}
                  />
                </div>
              </div>
          ...
          if (nodeType === NodeTypeId.RunWorkflow) {
            ...
            const updateRwParams = (updates: Record<string, unknown>) =>
              setEditingNodeData({
                ...editingNodeData,
                node: { ...editingNodeData.node, parameters: { ...rwParams, ...updates } },
              })
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1.5">
                    <Combobox value={rwWorkflowId || null} onValueChange={handleWorkflowSelect}>
                      <ComboboxInput
                  ...
                  {!rwWorkflowId && <p className="text-xs text-amber-500">Required — select the workflow to call.</p>}
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={editingNodeData.node.description || ""}
                    onChange={(e) => handleNodeChange({ description: e.target.value })}
                    placeholder="Why is this sub-workflow being called here?"
                    rows={2}
                  />
                </div>
              </div>
        ...
        if (activeTab === "input") {
              ...
              <div className="space-y-4">
                ...
                {!hasSchema ? (
                  ...
                  <ToolParameterForm
                    ...
                    onChange={(updated) =>
                      setEditingNodeData({ ...editingNodeData, node: { ...editingNodeData.node, parameters: updated } })
                    }
                ...
                {hasSchema && (
                  <div className="space-y-2 pt-1">
                    <Label>
                      Agent instructions <span className="text-muted-foreground font-normal">(optional)</span>
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      Tell the agent what to do and how to derive any agent-populated values. Type{" "}
                      <code className="font-mono">$</code> to reference prior node outputs.
                    </p>
                    <PromptInput
                      ...
                      placeholder={
                        'e.g. "Ask the user which of the options produced in the previous step they want to proceed with."'
                      }
                      rows={4}
                    />
                  </div>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Define the JSON shape and display for each field. Fields are referenced as{" "}
                      <code className="font-mono">
                        ${"$"}
                        {(editingNodeData.node as any).key || "node_key"}.<i>field</i>
                      </code>
                      .
                    </p>
                  </div>
                    ...
                    if (
                      ...
                      return (
                        <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2">
                          Schema root type is <code className="font-mono">{String(rawSchema.type)}</code> — the builder
                          only supports object roots. Add a field to reset to a compatible schema.
                        </p>
                      )
                    }
                    return null
                  ...
                  <SchemaBuilder
                    props={
                      ...
                      jsonSchemaToProps(
                        (editingNodeData.outputSchema as Record<string, unknown>) ?? { type: "object", properties: {} },
                      )
                    }
          ...
          if (nodeType === NodeTypeId.Parameters) {
            ...
            const updateWfParams = (updated: WorkflowParameter[]) =>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Visible to triggers and the LLM as <code className="font-mono">$input.*</code>
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      onClick={() =>
                        updateWfParams([...params, { name: "", type: "string", description: "", required: true }])
                      }
                    >
                      ...
                      <div key={i} className="border rounded-md p-3 space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Select
                            ...
                            onValueChange={(v) => {
                              const u = [...params]
                              u[i] = { ...u[i], type: v as JsonSchemaType }
                              updateWfParams(u)
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => updateWfParams(params.filter((_, j) => j !== i))}
                          >
                        ...
                        <ExpressionInput
                          ...
                          suggestions={availableRefs ?? []}
                          placeholder="Description — shown to the LLM when this workflow runs. Type $ to reference upstream outputs."
                          className="text-xs"
                        />
                        {(["string", "number", "integer"] as const).includes(
                          (param.type ?? "string") as "string" | "number" | "integer",
                        ) && (
                          <div className="space-y-1">
                            <span className="text-xs text-muted-foreground">
                              Allowed values <span className="opacity-60">(empty = accept any)</span>
                            </span>
                            <EnumTagInput
                              values={param.enum ?? []}
                              onChange={(next) => {
                                const u = [...params]
                                u[i] = { ...u[i], enum: next.length > 0 ? next : undefined }
                                updateWfParams(u)
                              }}
                            />
                          </div>
            ...
            if (mode === "agent") {
              return (
                <div className="py-6 text-center space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Switch to <strong>Deterministic</strong> mode in General if you want to evaluate an expression
                    instead.
                  </p>
                </div>
              )
                ...
                <div className="space-y-2">
                  ...
                  {!inputExpr && (
                    <p className="text-xs text-amber-500">
                      Required — without this the workflow will error at runtime. Type{" "}
                      <code className="font-mono">$</code> to pick from upstream outputs.
                    </p>
                  )}
                  {inputExpr && (
                    <p className="text-xs text-muted-foreground">
                      Resolved value is compared against each condition in the Settings tab.
                    </p>
                  )}
                </div>
              </div>
            ...
            return (
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  {!pathVal && (
                    <p className="text-xs text-amber-500">
                      Required — the directory to set. Type <code className="font-mono">$</code> to reference an
                      upstream value.
                    </p>
                  )}
                  {pathVal && (
                    <p className="text-xs text-muted-foreground">
                      Session working directory will be updated to this path. All subsequent nodes inherit it.
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  <Label>
                    Output key <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <Input
                    value={outputKey}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    If set, the resolved path is also stored as <code className="font-mono">$ctx.&lt;key&gt;</code> for
            ...
            return (
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Sets the session&apos;s active model from this node forward. Subsequent nodes inherit it until
                    changed again or reset to &quot;Agent default&quot;.
                  </p>
                </div>
                <div className="space-y-2">
                  <Label>
                  ...
                  <ExpressionInput
                    value={cwdVal}
                    onChange={(v) => updateParams({ cwd: v || undefined })}
                    suggestions={availableRefs}
                    placeholder="$ctx.project_directory"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <ExpressionInput
                    value={titleVal}
                    onChange={(v) => updateParams({ title: v || undefined })}
                    suggestions={availableRefs}
                    placeholder="Leave blank to keep current title, or use $nodeKey.field"
                  />
                </div>
                <div className="space-y-2">
                  <Label>
                    Agent <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <Combobox
                  ...
                  >
                    ...
                    <ComboboxContent>
                      ...
                      <ComboboxList>
                        {agentList.map((a) => (
                          <ComboboxItem key={a.id ?? a.name} value={a.id ?? a.name}>
                            {a.name}
                          </ComboboxItem>
                        ))}
                      </ComboboxList>
                ...
                <div className="space-y-2">
                  <Label>
                    Write path boundary <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <ExpressionInput
                    value={pathVal}
                    onChange={(v) => updateParams({ path: v || undefined })}
                    suggestions={availableRefs}
                    placeholder="$ctx.write_path"
                  />
                </div>
                <div className="space-y-2">
                  <Label>
                    Read path boundary <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <ExpressionInput
                    value={readPathVal}
                    onChange={(v) => updateParams({ readPath: v || undefined })}
                    suggestions={availableRefs}
                    placeholder="$ctx.read_path"
                  />
                </div>
              </div>
            )
            ...
            return (
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {inlineItems.length === 0 && (
                      <p className="text-xs text-muted-foreground py-2 text-center">
                        No items yet. Add items below — each one can reference upstream outputs using{" "}
                        <code className="font-mono">$nodeKey.field</code>.
                      </p>
                    )}
                    <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Each item runs one loop iteration. Use <code className="font-mono">$nodeKey.field</code> to embed
                      dynamic values from earlier nodes.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Reference an upstream array. String elements may contain <code className="font-mono">$ref</code>{" "}
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Each element is injected as <code className="font-mono">$ctx.{itemVar || "item"}</code> inside the
                    loop body.
                  </p>
                </div>
                <div className="space-y-2">
                  <Label>
                    Collect key <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <Input
                    value={collectVal}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    After each iteration, read <code className="font-mono">$ctx.{collectVal || "<collect>"}</code> and
                    append it to the results array. Leave blank to collect the item itself as a passthrough.
                  </p>
                </div>
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    The collected results array is stored as{" "}
                    <code className="font-mono">$ctx.{outputKey || "results"}</code> for downstream nodes.
                  </p>
                </div>
              </div>
            )
          ...
          if (nodeType === NodeTypeId.Output) {
            ...
            const updateFields = (updated: Record<string, string>) =>
              setEditingNodeData({
                ...editingNodeData,
                node: { ...editingNodeData.node, parameters: { ...params, fields: updated } },
              })
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Each field becomes a key in the returned <code className="font-mono">result</code> object.
                      </p>
                    </div>
                    <Button
                      variant="outline"
                  ...
                  {fieldEntries.length === 0 && (
                    <p className="text-xs text-muted-foreground py-3 text-center">
                      No fields yet. Add one to declare the workflow&apos;s return value.
                    </p>
                  )}
                  <div className="space-y-2">
          ...
          if (nodeType === NodeTypeId.Variable) {
            ...
            const setVariables = (updated: VarEntry[]) =>
              setEditingNodeData({
                ...editingNodeData,
                node: { ...editingNodeData.node, parameters: { ...params, variables: updated } },
              })
                  ...
                  <div key={i} className="border rounded-md p-3 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="font-mono text-xs flex-1"
                      />
                      <Select value={entry.type ?? "string"} onValueChange={(v) => updateEntry(i, { type: v })}>
                        <SelectTrigger className="w-28 text-xs">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
            ...
            if (!workflowId) {
              return (
                <div className="py-6 text-center">
                  <p className="text-xs text-muted-foreground">
                    Select a workflow in the General tab to configure its inputs.
                  </p>
                </div>
              )
            }
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Completion summary stored as{" "}
                    <code className="font-mono">$ctx.{outputKey || "workflow_result"}</code>.
                  </p>
                </div>
              </div>
            )
        ...
        if (activeTab === "settings") {
                        ...
                        onChange={(e) =>
                          handleNodeChange({
                            retry: {
                              ...
                              delaySeconds: editingNodeData.node.retry?.delaySeconds ?? 0,
                            },
                          })
                        }
                      />
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleNodeChange({
                            retry: {
                              maxAttempts: editingNodeData.node.retry?.maxAttempts ?? 0,
          ...
          if (nodeType === NodeTypeId.Prompt) {
            const ptc =
              ((editingNodeData as any).promptToolChoice as "auto" | "required" | "none" | undefined) ?? "auto"
            return (
              <div className="space-y-4">
                <div className="space-y-2">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Each condition is a named branch. Connect an edge and assign it.
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      ...
                      onClick={() => {
                        ...
                        const newCase =
                          mode === "deterministic" ? { label, when: { op: "equals", value: label } } : { label }
                  ...
                  {cases.length === 0 && (
                    <div className="border border-dashed rounded-md py-4 text-center">
                      <p className="text-xs text-muted-foreground">
                        No conditions yet — draw an edge from this node or add one above.
                      </p>
                    </div>
                  )}
                    ...
                    {cases.map((c, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            updated[i] = {
                              ...updated[i],
                              label: newLabel,
                              ...(mode === "deterministic" && currentOp === "equals"
                                ? { when: { op: currentOp, value: newLabel } }
                                : {}),
                            }
                        ...
                        {mode === "deterministic" && (
                          <>
                            <Select
                              ...
                              onValueChange={(op) => {
                                const u = [...cases]
                                u[i] = { ...u[i], when: { op, value: u[i].when?.value ?? "" } }
                                updateDecideParams({ cases: u })
                              }}
                            >
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                const u = [...cases]
                                u[i] = { ...u[i], when: { op, value: e.target.value } }
                                updateDecideParams({ cases: u })
                              }}
                              placeholder="value"
                              className="font-mono text-xs flex-1"
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-7 w-7 shrink-0 text-muted-foreground hover:text-destructive"
                          onClick={() => updateDecideParams({ cases: cases.filter((_, j) => j !== i) })}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      updateDecideParams(
                        checked ? { hasElse: true, default: "else" } : { hasElse: false, default: undefined },
                      )
                    }
                  />
                </div>
        ...
        if (activeTab === "output") {
                ...
                {outputs.length === 0 ? (
                  <div className="border border-dashed rounded-md py-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Outputs become available as <code className="font-mono">$ctx.*</code> in downstream nodes.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-1.5">
          ...
          if (nodeType === NodeTypeId.Decide) {
            ...
            return (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  The chosen label is automatically stored here. Use{" "}
                  <code className="font-mono">${`output.${autoKey}`}</code> in any downstream node. Rename the node in
                  General to change the key.
                </p>
              </div>
            )
          }
          ...
          if (nodeType === NodeTypeId.ForEach) {
            ...
            return (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  After all iterations complete, the collected results array is available as{" "}
                  <code className="font-mono">{`$ctx.${outputKey}`}</code> for downstream nodes. Change the key in the
                  Input tab.
                </p>
              </div>
            )
          ...
          if (nodeType === NodeTypeId.RunWorkflow) {
            ...
            return (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  The sub-workflow&apos;s completion summary is stored here. Use{" "}
                  <code className="font-mono">{`$ctx.${outputKey}`}</code> in downstream nodes. Change the key in the
                  Input tab.
                </p>
              </div>
            )
          ...
          if (nodeType === NodeTypeId.Variable) {
            ...
            const params = (editingNodeData.node.parameters ?? {}) as Record<string, unknown>
            const variables = (Array.isArray(params.variables) ? params.variables : []) as Array<{
              name: string
              type: string
            }>
            ...
            return (
              <div className="space-y-3">
                    ...
                    {variables
                      ...
                      .map((v) => (
                        <div key={v.name} className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-muted/20">
                          <span className="font-mono text-xs flex-1 text-foreground">
                            ${nodeKey}.{v.name}
                          </span>
                ...
                <p className="text-xs text-muted-foreground">
                  Reference individual values as <code className="font-mono">${nodeKey}.&lt;name&gt;</code> in
                  downstream nodes. The full object is available as <code className="font-mono">${nodeKey}</code>.
                </p>
              </div>
            )
          }
          ...
          <div className="space-y-4">
            {formData.edgeLabel !== undefined && sourceDecideCases && (
              <div className="space-y-2">
                <Label>Branch</Label>
                <p className="text-xs text-muted-foreground">
                  Pick a case to route conditionally, or <code className="font-mono">result</code> to always fire and
                  pass the decision value downstream.
                </p>
    ...
    >
      ...
      <div className="absolute -top-16 inset-x-0 flex justify-center z-[100] pointer-events-none">
        <div className="relative pointer-events-auto">
          ...
          <WorkflowControls orientation="horizontal" showInteractive={true} className="!static shadow-lg">
            {setShowMiniMap && (
              <WorkflowControlButton onClick={() => setShowMiniMap(!showMiniMap)} title="Toggle Minimap">
