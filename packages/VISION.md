...
The core packages in `packages/` are the bare-metal application. All optional capabilities — contributed by plugins from the `projectflows-website/registry` catalog — are installed under `.projectflows/plugins/installed/<plugin-id>/` and loaded at runtime. They are never bundled into `packages/`.
