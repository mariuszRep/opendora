 ...
 *
 * Mirrors the openclaw agent CRUD pattern:
 *  - index.json is the registry (equivalent to config.json's agents.list[])
 *  - applyToIndex / pruneFromIndex are the mutation helpers
 *  - Filesystem preflight (mkdir + file writes) happens BEFORE index write
 *  - File access is security-constrained: whitelisted names, path boundary,
 *    symlink rejection, hardlink rejection
 */
import z from "zod"
...
export namespace AgentFile {
  const log = Log.create({ service: "agent-file" })
  ...
  /** Stored inside each agent dir as agent.json */
  ...
  function agentsRoot(): string {
    return path.join(Instance.directory, OPENDORA_DIR, AGENTS_SUBDIR)
  }
  ...
  function indexPath(): string {
    return path.join(agentsRoot(), INDEX_FILE)
  }
   ...
   */
  function resolveAllowed(agentId: string, filename: string): string {
   ...
   */
  async function safeRead(agentId: string, filename: string): Promise<string> {
    const target = resolveAllowed(agentId, filename)
   ...
   */
  async function safeWrite(agentId: string, filename: string, content: string): Promise<void> {
    const target = resolveAllowed(agentId, filename)
  ...
  export async function loadAll(): Promise<Entry[]> {
    await ensureRoot()
   ...
   */
  export async function load(id: string): Promise<Entry> {
   ...
   */
  export async function create(id: string, config: Config, persona = ""): Promise<Entry> {
    await ensureRoot()
