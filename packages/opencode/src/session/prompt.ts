...
export namespace SessionPrompt {
  ...
  export const loop = fn(LoopInput, async (input) => {
    ...
    while (true) {
      ...
      if (task?.type === "subtask") {
        ...
        let taskAgent = await Agent.getByIdOrName(task.agent)
        if (!taskAgent) {
          // Fallback to default agent if stored reference is invalid
          const defaultAgentName = await Agent.defaultAgent()
          taskAgent = await Agent.get(defaultAgentName)
          if (!taskAgent) {
            throw new Error(`Agent "${task.agent}" not found and default agent "${defaultAgentName}" is also unavailable`)
          }
          Log.Default.warn("subtask agent not found, using default", { 
      ...
      // normal processing
      let agent = await Agent.getByIdOrName(lastUser.agent)
      if (!agent) {
        // Fallback to default agent if stored reference is invalid
        const defaultAgentName = await Agent.defaultAgent()
        agent = await Agent.get(defaultAgentName)
        if (!agent) {
          throw new Error(`Agent "${lastUser.agent}" not found and default agent "${defaultAgentName}" is also unavailable`)
        }
        Log.Default.warn("agent not found, using default", { 
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (input.agent.config?.enableInjection) {
      const injection = await Agent.getInjection(input.agent.id)
      if (injection) {
    ...
    // Entering plan mode - injection handled by generic injection system
    if (input.agent.name === "plan" && assistantMessage?.info.agent !== "plan") {
