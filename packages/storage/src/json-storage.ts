...
export namespace Storage {
  ...
  const MIGRATIONS: Migration[] = [
    async (dir) => {
      ...
      if (!(await fs.stat(project).then((s) => s.isDirectory()).catch(() => false))) return
      ...
      for (const projectDir of projectDirs) {
        ...
        if (!(await fs.stat(fullPath).then((s) => s.isDirectory()).catch(() => false))) continue
        ...
        if (projectID !== "global") {
          ...
          if (!(await fs.stat(worktree).then((s) => s.isDirectory()).catch(() => false))) continue
