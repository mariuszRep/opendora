...
// Simple error class to replace NamedError
class RipgrepError extends Error {
  constructor(public code: string, public data: Record<string, any>) {
    super(code)
    this.name = code
  }
}
...
export namespace Ripgrep {
  const log = { info: (...args: any[]) => {}, warn: (...args: any[]) => {}, error: (...args: any[]) => {} }
  ...
  export class ExtractionFailedError extends RipgrepError {
    constructor(data: { filepath: string; stderr: string }) {
      super("RipgrepExtractionFailedError", data)
    }
  }

  export class UnsupportedPlatformError extends RipgrepError {
    constructor(data: { platform: string }) {
      super("RipgrepUnsupportedPlatformError", data)
    }
  }

  export class DownloadFailedError extends RipgrepError {
    constructor(data: { url: string; status: number }) {
      super("RipgrepDownloadFailedError", data)
    }
  }

  let binaryPath: string | undefined
  
  export function setBinaryPath(binDir: string) {
    binaryPath = binDir
  }
