...
import { WebFetchTool } from "@projectflows/tools/browse-and-web/webfetch"
      ...
      async () => {
        await Instance.provide({
          ...
          fn: async () => {
            ...
            expect((result.attachments?.[0] as any).mime).toBe("image/png")
            expect((result.attachments?.[0] as any).url.startsWith("data:image/png;base64,")).toBe(true)
