...
export const ToolUpdateTool = Tool.define("tool_update", async (_initCtx) => {
    ...
    async execute(args: z.infer<typeof parameters>, ctx) {
      if (!args.description && !args.parameter_descriptions) {
        return {
          ...
          metadata: { name: args.name, file: undefined as string | undefined, changes: 0 },
      ...
      if (args.parameter_descriptions) {
        ...
        if (unknown.length) {
          return {
            ...
            metadata: { name: args.name, file: tool.file, changes: 0 },
      ...
      return {
        ...
        metadata: { name: args.name, file: tool.file as string | undefined, changes: changes.length },
